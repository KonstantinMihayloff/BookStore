...
// Number of books returned by the API after each sync
...
private val BOOKS = listOf(
    ...
    Book(0, "The Hound Of the Baskerville", "Arthur Conan Doyle",
...
class FakeBookApi {
    ...
    // Function used to load the books 2 by 2 as the following way
    // Book 1 and 2
    // then books 3 and 4
    // then books 5 and 6
    // We use % computation here to go through this cycle
    fun loadBooks() : List<Book> {
        val booksSize = BOOKS.size

        // New book index to load
        var newBookIndex = (bookIndex + INDEX_OFFSET) % booksSize

        // If all 6 books are loaded, then we have to start the cycle again
        if(newBookIndex == 0) {
